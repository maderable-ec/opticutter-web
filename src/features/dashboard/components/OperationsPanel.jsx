...
const OperationsPanel = ({ from, to }) => {
  const { data, isLoading, error } = useOperations(from, to)

  if (isLoading) {
    return (
      <CCard className="mb-4">
        <CCardHeader className="small fw-semibold text-body-secondary">Operación y eficiencia</CCardHeader>
  ...
  const lifecycleChart = lifecycle.length > 0 ? (
    <CChartBar
      style={{ height: '220px' }}
      data={{
        labels: lifecycle.map((t) => {
          const from = TRANSITION_LABEL[t.fromStatus] ?? t.fromStatus
          const to = TRANSITION_LABEL[t.toStatus] ?? t.toStatus
          return `${from} → ${to}`
        }),
        datasets: [
          {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => {
                ...
                return [`Tiempo: ${fmtHours(t.avgHours)}`, `Muestra: ${t.sampleCount} transiciones`]
              },
            },
          },
        },
        ...
        <CRow className="g-3 mb-4">
          ...
          <CCol xs={6}>
            <div className="border-start border-start-4 border-start-warning py-1 px-3">
              ...
              <div className="fs-3 fw-bold text-warning">{fmtPct(data.expiryBeforeApprovalRate)}</div>
